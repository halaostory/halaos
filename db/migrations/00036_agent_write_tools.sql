...
-- +goose Down
UPDATE agents SET system_prompt = regexp_replace(system_prompt, E'\n\nWRITE ACTION SAFETY RULES:.*', '', 's')
WHERE slug IN ('general', 'leave', 'attendance');

UPDATE agents SET tools = array_remove(array_remove(array_remove(array_remove(array_remove(tools, 'list_leave_types'), 'create_leave_request'), 'clock_in'), 'clock_out'), 'create_overtime_request')
WHERE slug IN ('general', 'leave', 'attendance');
