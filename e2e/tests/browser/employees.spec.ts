...
test.describe('Employees List', () => {
  test('page loads at /employees with table visible', async ({ adminContext }) => {
  ...
  test('search input exists', async ({ adminContext }) => {
    const page = await adminContext.newPage();
    await page.goto(BASE + '/employees');
    await page.waitForLoadState('networkidle');
  ...
  test('add employee button visible for admin', async ({ adminContext }) => {
    const page = await adminContext.newPage();
    await page.goto(BASE + '/employees');
    await page.waitForLoadState('networkidle');
